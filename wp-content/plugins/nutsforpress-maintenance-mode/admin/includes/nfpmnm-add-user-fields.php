...
if(!function_exists('nfpmnm_add_user_fields')){
	...
	function nfpmnm_add_user_fields($nfpmnm_user) {
		...
		if(
			...
			?>
			<h2><?php echo __('Login on Maintenance Mode', 'nfpmnmlang'); ?></h2>

			<table class="form-table">
				<tbody>
					
					<tr>
						<th><label for="nfpmnm-allow-login"><?php echo __('Allow Login', 'nfpmnmlang'); ?></label></th>
						<td>
							...
							echo '<label for="nfpmnm-login-on-maintenance">'.__('This Administrator can keep on working and login even when Maintenance Mode is switched on', 'nfpmnmlang').'</label>';
							echo '<p style="margin-top: 15px"><em>'.__('If no Administrators have this checkbox flagged, all of them can keep on working and login even if the Maintenance Mode is switched on; if one or some Administrators have this checkbox flagged, the others are instantly logged out and they are also prevented from login', 'nfpmnmlang').'.</em></p>';
